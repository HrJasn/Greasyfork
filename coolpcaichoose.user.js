...
// @version      2026-03-15.12
// @description  在原價屋估價頁面讓AI自動抓取當下選項配單。新增自動重試開關、修復動態降級跳號問題、擴大自動重試的容錯範圍。
...
(function() {
    ...
    autoSwitchCheckbox.checked = GM_getValue('auto_switch_model', true); // 預設開啟
    ...
    minimizedIcon.addEventListener('click', () => {
        isMinimized = false; GM_setValue('gui_is_minimized', false); toggleView();
    });

    minimizeBtn.addEventListener('click', () => {
        isMinimized = true; GM_setValue('gui_is_minimized', true); toggleView();
    });
    ...
    clearKeyBtn.addEventListener('click', () => {
        const provider = providerSelect.value;
        if (provider === 'gemini') GM_setValue('gemini_api_key', '');
        else GM_setValue('openai_api_key', '');
        ...
        cachedModels.forEach(m => {
            if (m.id.toLowerCase().includes(lowerKeyword) || (m.name && m.name.toLowerCase().includes(lowerKeyword))) {
                ...
                if (currentSelectedId) {
                    if (m.id === currentSelectedId) opt.selected = true;
                } else {
    ...
    function toggleModelExhausted(modelId, isExhausted) {
        ...
        if (model && model.exhausted !== isExhausted) {
            model.exhausted = isExhausted;
            renderModels(modelSearchInput.value);
        }
    }
    ...
    function getNextBestModel() {
        ...
        function getModelScore(modelId) {
            ...
            if (id.startsWith('o1')) score += 50;
            if (id.startsWith('o3')) score += 60;
            if (id.includes('gpt-4o')) score += 5;
            if (id.includes('turbo')) score += 2;
            if (id.includes('mini')) score -= 1;

            if (id.includes('ultra')) score += 3;
            if (id.includes('pro')) score += 2;
            if (id.includes('flash')) score += 1;
            if (id.includes('8b')) score -= 1;

            if (/\d{4}$/.test(id) || id.includes('exp')) score -= 0.5;
            if (id.includes('vision')) score -= 1;
    ...
    loadModelsBtn.addEventListener('click', () => {
        ...
        const handleSuccess = (modelsArr) => {
            ...
            cachedModels = modelsArr;
            renderModels('');

            modelSelectWrapper.style.display = 'block';
            modelSearchWrapper.style.display = 'block';
            statusText.innerText = `狀態：成功載入 ${cachedModels.length} 個模型！您可以開始輸入需求了。`;
            statusText.style.color = 'green';

            engineSection.collapse();
            modelSection.collapse();
        };
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(response) {
                    try {
                        ...
                        loadModelsBtn.disabled = false;
                        statusText.innerText = `API 錯誤: ${e.message || '無法解析回應'}`;
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(response) {
                    try {
                        ...
                        .map(m => ({ id: m.id, name: m.id, exhausted: false }))
...
【重要任務：追問修改】：如果使用者需求是針對「目前的配單」進行修改，請你務必保留「目前已選擇零件」中不需要更動的部分，僅替換使用者要求修改的項目，並重新考量整體相容性。最後回傳完整的配單。
...
只需回傳有選到的分類即可。`;
            ...
            function sendApiRequest(targetModelId) {
                ...
                if (provider === 'gemini') {
                    GM_xmlhttpRequest({
                        method: 'POST',
                        url: `https://generativelanguage.googleapis.com/v1beta/${targetModelId}:generateContent?key=${apiKey}`,
